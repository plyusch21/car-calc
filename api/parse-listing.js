...
// свой путь — VISION_READ_PROMPT + SHEET_TO_FIELDS_PROMPT, объявлены дальше.
...
// Разбор фото идёт В ДВА ШАГА, и это не усложнение ради усложнения — так
// показал живой тест на реальном листе владельца.
//
// Одним шагом (фото + схема сразу) модель отдавала всего три поля: марку,
// год и объём. А тот же файл, тем же ключом, той же моделью, но с просьбой
// просто прочитать лист вслух — дал развёрнутое чтение: номер лота, код
// кузова, пробег 39 346 км, топливо, цвет, число мест, оценки 3.5/B,
// замечания инспектора и даже расшифровку схемы повреждений (A1 на
// бампере, W2 = перекрас задней двери), плюс честную пометку, где почерк
// неразборчив. То есть читать она умеет прекрасно — теряется именно на
// одновременном "прочитай И сразу разложи в жёсткий JSON": под схемой у
// неё нет места рассуждать, а расшифровка схемы повреждений — это именно
// рассуждение. Плюс прежний промпт пять раз повторял "не уверен — оставь
// пустым", и на фото, где абсолютной уверенности не бывает никогда, это
// давило в молчание.
//
// Поэтому: шаг 1 — свободное чтение (то, что модель делает отлично),
// шаг 2 — разложить уже прочитанный ТЕКСТ по полям, а это обычная
// текстовая задача, которая у нас и так работает хорошо.
const VISION_READ_PROMPT = `Перед тобой фотография японского аукционного листа (オークションシート).
...
Если лист есть — прочитай его целиком и перескажи по-русски всё, что на нём написано: марку, модель, грейд, код кузова, номер кузова, дату первой регистрации, пробег, объём двигателя, тип КПП, привод, топливо, цвет, оценку листа и салона, опции, замечания продавца и инспектора, пометки на схеме кузова с расшифровкой.

Пиши вслух и подробно, это черновик для дальнейшего разбора, а не готовый ответ клиенту. Если какая-то часть читается плохо — так и напиши, что именно и как её можно прочитать. Не выдумывай того, чего не видишь.`;

// Шаг 2: прочитанное превращаем в поля. Здесь уже нет картинки — обычный
// текст, с которым модель справляется без проблем, поэтому и тон другой:
// не "бойся ошибиться", а "разложи аккуратно". Про схему повреждений и
// замечания инспектора сказано явно — это самое ценное для поля condition,
// и именно его раньше не было вообще.
const SHEET_TO_FIELDS_PROMPT = `Ниже — подробное чтение японского аукционного листа. Разложи его по полям схемы.

Куда что класть:
- condition — состояние автомобиля: замечания инспектора и расшифровка схемы повреждений (царапины, вмятины, сколы, перекрасы), кратко и по-русски.
- notes — остальное существенное, для чего нет отдельного поля: оценка листа и салона, цвет, число мест, заметные опции, сторона руля, номер кузова. Не дублируй то, что уже попало в другие поля.
- model и carTrim — на английском. Дата — именно первой регистрации, а не срок техосмотра.

Если в тексте сказано, что какое-то место читалось плохо или неоднозначно, но смысл всё же понятен — используй его. Если значения нет вовсе — оставь поле пустым.
...
async function callGeminiVision(base64Data, mimeType) {
  // Шаг 1 — свободное чтение листа (см. большой комментарий у VISION_READ_PROMPT).
  const reading = await geminiGenerate({
    contents: [{ parts: [
      { text: VISION_READ_PROMPT },
  ...
  // Шаг 2 — разложить прочитанное по полям. Картинки здесь уже нет.
  const parsed = await callGeminiRaw(
    [{ text: SHEET_TO_FIELDS_PROMPT.replace('{{TEXT}}', () => reading.slice(0, 12000)) }],
    FUNCTION_SCHEMA.parameters
  );
  parsed.source = 'ai';
