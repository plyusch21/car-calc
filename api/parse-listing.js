...
// короткий промпт — GEMINI_VISION_PROMPT, он объявлен дальше по файлу.
...
// Промпт для Gemini: коротко и по делу. Японский он знает лучше, чем мы
// сумеем ему пересказать, поэтому никаких списков "что означает какая
// подпись" — только сама задача и рамки, которые нам действительно нужны:
// не выдумывать, не путать дату регистрации с техосмотром (от этого прямо
// зависит расчёт), язык полей на выходе и напоминание читать весь лист, а
// не только крупные надписи сверху (именно этим Gemini и грешил — отдавал
// марку/год/объём и на этом останавливался).
const GEMINI_VISION_PROMPT = `Ты читаешь японский аукционный лист (オークションシート) так, как его читает японец, и свободно владеешь русским языком.

Сначала проверь: на фото действительно аукционный лист, и его видно достаточно, чтобы читать? Если нет (другое изображение, случайный скриншот, размытое или нечитаемое фото) — верни isAuctionSheet=false, все остальные поля пустыми и ничего не придумывай.

Если лист читается — верни isAuctionSheet=true и заполни поля схемы тем, что на нём реально написано.

Главное правило: значение попадает в поле, только если ты уверен сразу в двух вещах — что верно его прочитал и что оно относится именно к этому полю. Прочитал число, но не понял, что именно оно означает — не клади его в поле, которое просто похоже по смыслу, оставь пустым. Пустое поле лучше угаданного или перепутанного: эти данные идут прямо в расчёт стоимости автомобиля.

Читай лист целиком, а не только крупные надписи сверху. Пробег, трансмиссия, привод, топливо, оценки листа и салона, комментарии инспектора, пометки о повреждениях и о дополнительных опциях — всё это тоже есть на листе и тоже нужно.

Даты: нам нужна дата первой регистрации автомобиля, а не срок действия техосмотра (車検) — не перепутай их. Год японской эры переведи в обычный.

Язык ответа: model и carTrim — на английском, condition и notes — на русском (переведи с японского). В notes собери существенное, для чего нет отдельного поля в схеме, не дублируя то, что уже попало в другие поля.`;
